    ...
    try {
      ...
      if (Value.trim()==="") {
        ...
        const filteredData = res.data.filter((el) =>
          el.name.toLowerCase().includes(Value.toLowerCase()) && el.type==="ROOM"
        );
        return filteredData;
      }
    } catch (err) {
    ...
    async (input, thunkApi) => {
      try {
        ...
        if (Value.trim()==="") {
          ...
          const filteredData = res.data.filter((el) =>
            el.name.toLowerCase().includes(Value.toLowerCase()) && el.type==="COMMON"
          );
          return filteredData;
        }
      } catch (err) {
...
const animitySlice = createSlice({
    ...
    .addCase(animityAsync.fulfilled, (state, action) => {
        ...
        state.animityCommon = action.payload.filter(el => el.type === "COMMON");
        if(state.searchValueRoom.trim() === "") state.animityRoomFilter = state.animityRoom
        else {state.animityRoomFilter = action.payload.filter((el) =>
        el.name.toLowerCase().includes(state.searchValueRoom.toLowerCase()) && el.type==="ROOM"
      )}
        if(state.searchValueCommon.trim() === "") state.animityCommonFilter = state.animityCommon
        else {state.animityCommonFilter = action.payload.filter((el) =>
        el.name.toLowerCase().includes(state.searchValueCommon.toLowerCase()) && el.type==="COMMON"
        )}
