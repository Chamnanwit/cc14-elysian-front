      ...
      <div className="grid gap-6 mb-6 md:grid-cols-2">
        <div>
          ...
          <select
            ...
            id="planType"
            className={`border ${
              error.expiration ? "border-red-500" : "border-gray-300"
            } text-gray-900 text-lg rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`}
            name="planType"
          ...
          >
            ...
            {pricingPlanType.map((el) =>
              el.type == input.planType ? (
                <option selected key={el?.id} value={el.type}>
                  {el.type}
                </option>
              ) : (
        ...
        <div>
          ...
          <select
            ...
            className={`border text-lg ${
              error.expiration ? "border-red-500" : "border-gray-300"
            } text-gray-900 text-md rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2`}
            name="expiration"
            onChange={handleChangeInput}
          >
            ...
            {expirationType.map((el) =>
              el.type === input.expiration ? (
                <option selected key={el?.id} value={el.type}>
                  {el.thaiType}
                </option>
              ) : (
          ...
          <div className="h-0 pb-2">
            {error.expiration && (
              <InputErrorMessage message={error.expiration} />
            )}
          </div>
        </div>
        <div>
        ...
        <div>
          <div>
            ...
            />
            {error.numberOfTop && (
              <InputErrorMessage message={error.numberOfTop} />
            )}
          </div>
        </div>
        <div>
          ...
          <select
            ...
            className={`border text-lg ${
              error.expiration ? "border-red-500" : "border-gray-300"
            } text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500`}
            name="locked"
            onChange={handleChangeInput}
          >
            <option hidden selected value={""}>
